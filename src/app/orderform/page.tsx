...
const allGarmentTypes = Array.from(new Set([
  ...Object.keys((measurementData as any).male || {}),
  ...Object.keys((measurementData as any).female || {}),
])) as [string, ...string[]];

const orderDetailsSchema = z.object({
  orderType: z.enum(allGarmentTypes, { required_error: "Select a garment type" }),
...
export default function OrderFormPage() {
  ...
  const [garmentGender, setGarmentGender] = useState<'male' | 'female' | undefined>(undefined);
  const handleGarmentGenderChange = (value: string) => setGarmentGender(value as 'male' | 'female');
  ...
  const scrollToGarmentsSummary = () => {
    if (garmentsSummaryRef.current) {
      garmentsSummaryRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  };
  ...
  const garmentOptions: Record<'male' | 'female', { value: string; label: string }[]> = {
    male: Object.keys((measurementData as any).male || {}).map(key => ({ value: key, label: key.charAt(0).toUpperCase() + key.slice(1) })),
    female: Object.keys((measurementData as any).female || {}).map(key => ({ value: key, label: key.charAt(0).toUpperCase() + key.slice(1) })),
  };
  ...
  const convertValue = (val: string | number, from: 'inches' | 'centimeters', to: 'inches' | 'centimeters'): string => {
  ...
  const handleUnitToggle = (newUnit: 'inches' | 'centimeters') => {
    ...
    const current = measurementForm.getValues("measurements") as Record<string, string | number> || {};
  ...
  const handleDeliverySubmit = async (data: DeliveryPayment) => {
    ...
    try {
      ...
      garments.forEach((g, idx) => {
        ...
        if (g.measurement.designReference && Array.isArray(g.measurement.designReference)) {
        ...
        if (g.measurement.canvasImage && typeof g.measurement.canvasImage === 'string' && g.measurement.canvasImage.startsWith('data:image/')) {
        ...
        // Voice Note (file)
        if (g.measurement.voiceNote && g.measurement.voiceNote instanceof File) {
      ...
      if (garments.length === 0) {
        if (measurementFiles && measurementFiles.length > 0) {
          measurementFiles.forEach((file, i) => {
            if (file instanceof File) formData.append(`designReference_0_${i}`, file);
          });
        }
        // --- FIX: Use local variable and safe checks for canvasImage ---
        const canvasImage = measurementForm.getValues().canvasImage;
        if (typeof canvasImage === 'string' && canvasImage.startsWith('data:image/')) {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 py-8 px-2">
      ...
      <Card className="w-full max-w-xl shadow-2xl transition-all duration-500 animate-fade-in px-2 sm:px-0">
        ...
        <CardContent className="px-0 sm:px-6">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <div className="flex items-center mt-1">
                        <input type="checkbox" id="sameForWhatsapp" checked={form.watch("sameForWhatsapp") || false} onChange={e => form.setValue("sameForWhatsapp", e.target.checked)} />
                        <label htmlFor="sameForWhatsapp" className="ml-2 text-xs">Same for WhatsApp</label>
                      </div>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Enter full address including pin code" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          {step === 2 && showGarmentForm && (
            <Form {...orderForm}>
              <form onSubmit={orderForm.handleSubmit((orderData) => {
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  {/* 3. In the garment form, add gender select before garment type */}
                    ...
                    <FormLabel>Gender *</FormLabel>
                      <FormControl>
                      <Select onValueChange={handleGarmentGenderChange} value={garmentGender}>
                          <SelectTrigger>
                  ...
                  <FormField control={orderForm.control} name="orderType" render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormLabel>Garment Type *</FormLabel>
                      <FormControl>
                        <Select onValueChange={field.onChange} value={field.value} disabled={!garmentGender}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select garment type" />
                          </SelectTrigger>
                          <SelectContent>
                            {(garmentGender ? garmentOptions[garmentGender] : []).map(opt => (
                  ...
                  <FormField control={orderForm.control} name="urgency" render={({ field }) => (
                    <FormItem className="flex-1">
                ...
                {garmentGender && garmentType && (
                  <div className="my-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <Button type="button" size="sm" variant={unit === 'inches' ? 'default' : 'outline'} onClick={() => handleUnitToggle('inches')}>Inches</Button>
                      <Button type="button" size="sm" variant={unit === 'centimeters' ? 'default' : 'outline'} onClick={() => handleUnitToggle('centimeters')}>Centimeters</Button>
                    </div>
                    {measurementFields.length === 0 ? (
                      <div className="text-gray-500 italic">No measurements required for this garment type.</div>
                    ) : (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                                <FormLabel className="text-sm">{fieldKey.replace(/([A-Z])/g, ' $1').replace(/^./, (s: string) => s.toUpperCase())}</FormLabel>
                      <FormControl>
                ...
                />
                  ...
                  <FormField control={measurementForm.control} name="designReference" render={({ field }) => (
                    <FormItem>
                      <FormLabel>Design Reference Images (JPG, PNG, max 5MB, up to 5 files)</FormLabel>
                      <FormControl>
                        <Input
                          type="file"
                  ...
                  <FormField control={measurementForm.control} name="designDescription" render={({ field }) => (
                    <FormItem>
                ...
                <FormField control={measurementForm.control} name="voiceNote" render={({ field }) => (
                  <FormItem>
            ...
            <Form {...deliveryForm}>
              <form onSubmit={deliveryForm.handleSubmit(handleDeliverySubmit)} className="space-y-5">
                ...
                <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between pt-4">
                  ...
                  <Button type="button" disabled={!deliveryForm.formState.isValid || garments.length === 0} onClick={() => setStep(4)} className="w-full sm:w-auto px-6 py-2 rounded-lg font-semibold shadow-md">
                    Next
                  </Button>
                </CardFooter>
                {submitSuccess && (
                  <div className="text-green-700 font-bold text-center mt-4 text-lg flex flex-col items-center gap-2">
                    ...
                    {orderOid && orderDate && (
                      <span className="bg-green-100 text-green-800 px-4 py-2 rounded-lg border border-green-300 mt-2">
                        Your Order ID: <span className="font-mono font-bold">{orderOid}</span><br />
                        Order Date: <span className="font-mono">{orderDate}</span>
                      </span>
                    )}
                  </div>
                )}
                {submitError && <div ref={errorRef} className="text-red-700 font-bold text-center text-lg my-4 border-2 border-red-300 bg-red-50 rounded p-3 shadow-lg">{submitError}</div>}
              </form>
            </Form>
          )}
          {step === 4 && (
                ...
                {submitSuccess ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <div className="mt-6 flex flex-col items-center gap-2">
                      <Button type="button" onClick={resetAll} className="px-6 py-2 rounded-lg font-semibold shadow-md">Submit another order</Button>
                      <span className="text-xs text-gray-500">or wait {resetTimer} seconds to reset</span>
                    </div>
                  </div>
                ) : (
                  <>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-sm">
                            ...
                            <div><span className="font-medium">Contact Number:</span> {customerData?.contactNumber}</div>
                            <div><span className="font-medium">Same for WhatsApp:</span> {customerData?.sameForWhatsapp ? "Yes" : "No"}</div>
                            <div><span className="font-medium">Email:</span> {customerData?.email}</div>
                            <div className="sm:col-span-2"><span className="font-medium">Full Address:</span> {customerData?.fullAddress}</div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-sm">
                            ...
                            <div><span className="font-medium">Design Description:</span> {measurementForm.getValues().designDescription}</div>
                          </div>
                                ...
                                {measurementForm.getValues().designReference.map((file: any, idx: number) => (
                                  <div key={idx} className="flex flex-col items-center">
                                    {file.type && file.type.startsWith("image/") ? (
                                      ...
                                      <span className=" h-20 w-20 bg-gray-200 rounded flex items-center justify-center border">PDF</span>
                                    )}
                        ...
                        {/* Garments Overview */}
                        <div>
                          <h3 className="font-semibold text-lg mb-1">Garments in Order</h3>
                          <ul className="space-y-2">
                            {garments.map((g, idx) => (
                              <li key={idx} className="border rounded p-2">
                                <div className="font-medium">{g.order.orderType.charAt(0).toUpperCase() + g.order.orderType.slice(1)} (Qty: {g.order.quantity})</div>
                                {/* 8. In the review step, show gender per garment */}
                                <div className="text-xs text-gray-600">Gender: {g.gender ? g.gender.charAt(0).toUpperCase() + g.gender.slice(1) : ''}</div>
                                <div className="text-xs text-gray-600">Urgency: {g.order.urgency}</div>
                                {/* Show measurements summary */}
                                <div className="mt-1 grid grid-cols-2 gap-1 text-xs">
                                  {Array.isArray(g.measurement.measurements) || !g.measurement.measurements
                                    ? null
                                    : Object.entries(g.measurement.measurements).map(([k, v]) => (
                                        <div key={k}><span className="font-medium">{k.replace(/([A-Z])/g, ' $1').replace(/^./, (s: string) => s.toUpperCase())}:</span> {String(v)}</div>
                                      ))}
                                </div>
                                {/* Show design reference images if present */}
                                {g.measurement.designReference && Array.isArray(g.measurement.designReference) && g.measurement.designReference.length > 0 && (
                                  <div className="mt-2">
                                    <div className="font-medium mb-1">Design Reference Images:</div>
                                    <div className="flex flex-wrap gap-2">
                                      {g.measurement.designReference.map((file: any, i: number) => (
                                        <img
                                          key={i}
                                          src={typeof file === 'string' ? file : URL.createObjectURL(file)}
                                          alt={file.name || `Design Reference ${i+1}`}
                    ...
                    <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between pt-4">
                      ...
                      <Button type="submit" className="w-full sm:w-auto px-6 py-2 rounded-lg font-semibold shadow-md" disabled={submitLoading}>
      ...
      {garments.length > 0 && (
        <div ref={garmentsSummaryRef} className="my-4 p-4 bg-slate-50 border rounded">
          <h3 className="font-semibold mb-2">Garments in Order:</h3>
          <ul className="space-y-2">
            {garments.map((g, idx) => (
              <li key={idx} className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between border-b pb-2">
                <div>
                  <span className="font-medium">{g.order.orderType.charAt(0).toUpperCase() + g.order.orderType.slice(1)}</span>
                  <span className="ml-2 text-xs text-gray-500">Qty: {g.order.quantity}</span>
                </div>
                <div className="flex gap-2">
                  <Button type="button" size="sm" variant="outline" onClick={() => handleEditGarment(idx)}>Edit</Button>
                  <Button type="button" size="sm" variant="destructive" onClick={() => handleRemoveGarment(idx)}>Remove</Button>
                </div>
              </li>
            ))}
          </ul>
