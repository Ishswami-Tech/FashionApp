...
const allGarmentTypes = Array.from(
  new Set(
    measurementData.measurement_new.map((form: any) => form.category)
  )
) as [string, ...string[]];
...
export default function OrderFormPage() {
  ...
  const printCustomerCopy = () => {
    ...
    const customerData = submittedOrder || {
      ...
      preferredCommunication: form.getValues().preferredCommunication
    };
    ...
    const customerContent = `
      ...
      <body>
        <div class="header">
          <h1 style="margin: 0; font-size: 16px;">📋 Your Order Confirmation</h1>
        ...
        <div class="section">
          ...
          <div class="highlight">
            ...
            <p style="margin: 2px 0;">• Delivery on ${deliveryData.deliveryDate?.toLocaleDateString?.() || String(deliveryData.deliveryDate)}</p>
    ...
    const adminContent = `
      ...
      <body>
        ...
        <div class="section">
          <div class="section-title">🎨 Design References</div>
          <div class="design-grid">
            ${garmentsData.map((garment: any, garmentIdx: number) => 
              garment.designs && Array.isArray(garment.designs) ? 
                garment.designs.map((design: any, designIdx: number) => `
                  <div class="design-item">
                    <p style="margin: 2px 0;"><strong>${garment.order?.orderType} - ${garment.variant}</strong></p>
        ...
        <div class="section">
          ...
          <div class="highlight">
            ...
            <p style="margin: 2px 0;">• Delivery: ${deliveryData.deliveryDate?.toLocaleDateString?.() || String(deliveryData.deliveryDate)}</p>
            <p style="margin: 2px 0;">• Order ID: ${orderId || submittedOrder?.oid}</p>
          </div>
        </div>
      </body>
      </html>
  ...
  const measurementFields: string[] = useMemo(() => {
    ...
    if (form.variants && form.variants.length > 0 && selectedVariant) {
      const variant = form.variants.find((v: any) => 
        typeof v === 'string' ? v === selectedVariant : v.type === selectedVariant
      );
      ...
      garments.forEach((g, idx) => {
        ...
        if (
          ...
          while (n--) u8arr[n] = bstr.charCodeAt(n);
              const file = new File([u8arr], `canvas_${idx}_${i}.png`, { type: mime });
              formData.append(`canvasImage_${idx}_${i}`, file);
        }
        // Voice Note (file)
        if (
              design.voiceNote &&
              design.voiceNote instanceof File
        ) {
  ...
  const totalAmount = garments.reduce((sum: number, g: any) => {
    if (g.designs && Array.isArray(g.designs)) {
      return sum + g.designs.reduce((dsum: number, d: any) => dsum + (parseFloat(d.amount) || 0), 0);
    }
    return sum;
  }, 0);
                            ...
                            {(() => {
                              ...
                              if (form && form.variants) {
                                return form.variants.map((variant: any) => (
                                  <SelectItem 
                                    key={typeof variant === 'string' ? variant : variant.type} 
                                    value={typeof variant === 'string' ? variant : variant.type}
                                  >
                                    {typeof variant === 'string' ? variant : variant.type}
                                </SelectItem>
                                ));
                              }
                              return [];
                ...
                {garmentType && (
                  <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-purple-900">Designs for Each Item</h3>
                    </div>
                    ...
                    {designs.map((d, idx) => (
                      <div key={idx} className="mb-6 p-4 border border-purple-200 rounded-lg bg-white shadow-sm">
                        <div className="flex items-center justify-between mb-3 pb-2 border-b border-purple-100">
                          <h4 className="font-semibold text-purple-900">Design #{idx + 1}</h4>
                          <span className="text-sm text-purple-600">Item {idx + 1} of {quantity}</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="block font-medium mb-1 text-sm text-gray-700">Design Name *</label>
                            <Input
                              type="text"
                          ...
                          <div>
                            <label className="block font-medium mb-1 text-sm text-gray-700">Amount (₹) *</label>
                            <Input
                              type="number"
                              min="0"
                              step="0.01"
                        ...
                        <div className="mt-4">
                          <label className="block font-medium mb-1 text-sm text-gray-700">Design Reference Images</label>
                          <Input
                            type="file"
                            ...
                            className="w-full"
                          />
                          <p className="text-xs text-gray-500 mt-1">Upload reference images for this design (optional)</p>
                              </div>
                        <div className="mt-4">
                          <label className="block font-medium mb-1 text-sm text-gray-700">Design Description</label>
                          <Textarea
                        ...
                        <div className="mt-4">
                          <label className="block font-medium mb-1 text-sm text-gray-700">Custom Drawing</label>
          ...
          {step === 3 && (
              ...
              >
                {submitLoading && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-lg font-semibold text-gray-700">Submitting your order...</p>
                    <p className="text-sm text-gray-500">Please wait while we process your order.</p>
                  </div>
                )}
                <div className={submitLoading ? "opacity-50 pointer-events-none" : ""}>
                <FormField
                      ...
                      g.designs && Array.isArray(g.designs) && g.designs.length > 0
                        ? g.designs.map((design: any, i: number) => (
                            <TableRow key={`${idx}-${i}`}>
                              ...
                              <TableCell>{design.name || `Design ${i + 1}`}</TableCell>
                ...
                <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between pt-4 w-full">
                  ...
                  <Button
                    ...
                    onClick={() => deliveryForm.handleSubmit(handleDeliverySubmit)()}
                    className="w-full sm:w-auto px-6 py-2 rounded-lg font-semibold shadow-md"
                  >
            ...
            <div className="p-4 sm:p-6 bg-white rounded shadow max-w-4xl mx-auto print:max-w-full print:shadow-none print:bg-white print:p-0">
              ...
              <style jsx>{`
                @media print {
                  * {
                  ...
                  .border-green-200, .border-blue-200, .border-purple-200, .border-orange-200 {
              ...
              <div className="print:break-after-page">
                <div className="text-center mb-6 pb-4 border-b-2 border-green-200">
                  ...
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Order Confirmed!</h2>
                  <p className="text-gray-600 mb-3">Your order has been successfully submitted</p>
                ...
                <div className="mb-6">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div className="text-gray-900 font-semibold">{submittedOrder?.fullName || customerData?.fullName}</div>
                            </div>
                            <div>
                        <div className="text-sm font-medium text-blue-700 mb-1">Contact Number</div>
                        <div className="text-gray-900 font-semibold">{submittedOrder?.contactNumber || customerData?.contactNumber}</div>
                            </div>
                            <div>
                        <div className="text-sm font-medium text-blue-700 mb-1">Email Address</div>
                        <div className="text-gray-900 font-semibold">{submittedOrder?.email || customerData?.email}</div>
                            </div>
                            <div>
                        <div className="text-sm font-medium text-blue-700 mb-1">Communication Preference</div>
                        <div className="text-gray-900 font-semibold capitalize">{submittedOrder?.preferredCommunication || customerData?.preferredCommunication}</div>
                            </div>
                      <div className="md:col-span-2">
                        <div className="text-sm font-medium text-blue-700 mb-1">Full Address</div>
                        <div className="text-gray-900 font-semibold">{submittedOrder?.fullAddress || customerData?.fullAddress}</div>
                            </div>
                            </div>
                          </div>
                        </div>
              ...
              <div className="print:break-after-page">
                <div className="text-center mb-6 pb-4 border-b-2 border-purple-200">
                  <h2 className="text-2xl sm:text-3xl font-bold text-purple-900 mb-2">Tailor Copy</h2>
                  ...
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 inline-block">
                    ...
                    <div className="text-xl sm:text-2xl font-mono font-bold text-purple-800">{submittedOrder?.oid || orderOid}</div>
                    <div className="text-xs text-purple-600 mt-1">Customer: {submittedOrder?.fullName || customerData?.fullName}</div>
                            </div>
                </div>
                ...
                {(submittedOrder?.garments || garments).map((garment: any, garmentIdx: number) => (
                  <div key={garmentIdx} className="mb-4 bg-purple-50 border border-purple-200 rounded-lg p-4 sm:p-6">
                    {/* Garment Header */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 pb-3 border-b border-purple-200">
                        ...
                        <p className="text-xs sm:text-sm text-purple-600">
                          Quantity: {garment.order?.quantity} • Urgency: {garment.order?.urgency}
                        </p>
                            </div>
                      <div className="text-right mt-2 sm:mt-0">
                        <div className="text-xs sm:text-sm text-purple-600">Total for this garment</div>
                        <div className="text-lg sm:text-xl font-bold text-purple-900">
                          ₹{garment.designs?.reduce((sum: number, d: any) => sum + (parseFloat(d.amount) || 0), 0).toFixed(2)}
                          </div>
                        </div>
                            </div>

                    {/* Designs */}
                    {garment.designs && Array.isArray(garment.designs) && garment.designs.length > 0 && (
                      <div className="space-y-3">
                        {garment.designs.map((design: any, designIdx: number) => (
                          <div key={designIdx} className="bg-white rounded-lg border border-purple-200 p-3 sm:p-4">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 sm:mb-3">
                              <h5 className="font-semibold text-purple-900 text-sm sm:text-base">
                                {design.name || `Design ${designIdx + 1}`}
                              </h5>
                              <div className="text-base sm:text-lg font-bold text-green-600 mt-1 sm:mt-0">₹{design.amount}</div>
                          </div>
                            
                            {/* Design Description */}
                            {design.designDescription && (
                              <div className="mb-2 sm:mb-3">
                                <div className="text-xs sm:text-sm font-medium text-gray-700 mb-1">Description</div>
                                <div className="text-gray-600 bg-gray-50 rounded p-2 text-sm">{design.designDescription}</div>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-2 sm:mb-3">
                              ...
                              {design.canvasImage && (
                                <div>
                                  <div className="text-xs sm:text-sm font-medium text-gray-700 mb-2">Canvas Drawing</div>
                                  <img
                              ...
                              {(design.designReferenceFiles && Array.isArray(design.designReferenceFiles) && design.designReferenceFiles.length > 0
                                ...
                                <div>
                                  <div className="text-xs sm:text-sm font-medium text-gray-700 mb-2">Reference Images ({(design.designReferenceFiles && design.designReferenceFiles.length) || (design.designReference && design.designReference.length)})</div>
                    ...
                    {/* Measurements */}
                    {garment.measurement?.measurements && Object.keys(garment.measurement.measurements).length > 0 && (
                      <div className="mt-3 pt-3 border-t border-purple-200">
                        <h5 className="font-semibold text-purple-900 mb-2 sm:mb-3 text-sm sm:text-base">Measurements</h5>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
                          {Object.entries(garment.measurement.measurements).map(([key, value]) => (
                            <div key={key} className="bg-white rounded-lg border border-purple-200 p-2 sm:p-3">
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4 text-center">
                  <div className="text-sm text-green-700 mb-1">Total Order Amount</div>
                  <div className="text-2xl sm:text-3xl font-bold text-green-800">₹{submittedOrder?.totalAmount || totalAmount.toFixed(2)}</div>
                          </div>
                        </div>
            </div>
            ...
            <div className="print:break-after-page">
              <div className="text-center mb-6 pb-4 border-b-2 border-orange-200">
                <h2 className="text-2xl sm:text-3xl font-bold text-orange-900 mb-2">Admin Copy</h2>
                <p className="text-orange-600 mb-3">Pricing & Payment Details</p>
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 inline-block">
                  ...
                  <div className="text-xl sm:text-2xl font-mono font-bold text-orange-800">{submittedOrder?.oid || orderOid}</div>
                  <div className="text-xs text-orange-600 mt-1">Customer: {submittedOrder?.fullName || customerData?.fullName}</div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 sm:p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      <h4 className="font-semibold text-orange-900 mb-2 sm:mb-3 text-sm sm:text-base">Delivery Information</h4>
                    ...
                    <div>
                      <h4 className="font-semibold text-orange-900 mb-2 sm:mb-3 text-sm sm:text-base">Payment Information</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-orange-700 text-sm">Payment Method:</span>
                          <span className="font-semibold capitalize text-sm">{submittedOrder?.payment || deliveryForm.getValues().payment}</span>
                                </div>
                        <div className="flex justify-between">
                          <span className="text-orange-700 text-sm">Amount:</span>
                          <span className="font-semibold text-green-600 text-sm">₹${submittedOrder?.totalAmount || totalAmount.toFixed(2)}</span>
                                      </div>
                                      </div>
                                    </div>
                                    </div>
                  ...
                  {(submittedOrder?.specialInstructions || deliveryForm.getValues().specialInstructions) && (
                    <div className="mt-3 pt-3 border-t border-orange-200">
                      <h4 className="font-semibold text-orange-900 mb-2 text-sm sm:text-base">Special Instructions</h4>
                      <div className="bg-white rounded-lg p-3 border border-orange-200">
                        <p className="text-gray-700 text-sm">{submittedOrder?.specialInstructions || deliveryForm.getValues().specialInstructions}</p>
                      </div>
                                  </div>
                                )}
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Garment</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Variant</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Designs</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {(submittedOrder?.garments || garments).map((garment: any, garmentIdx: number) => (
                        <tr key={garmentIdx}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-500">
                            {garment.designs?.length || 0} design{garment.designs?.length !== 1 ? 's' : ''}
                          </td>
              ...
              <div className="print:hidden">
                <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <h3 className="font-semibold text-gray-900 mb-3">Print Options:</h3>
              ...
              <div className="mt-4 sm:mt-6 text-center">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Order ID: {submittedOrder?.oid || orderOid} • Date: {submittedOrder?.orderDate || orderDate}
                </p>
              </div>
                      </div>
          )}
